# DISPATCH — Worker U4 (Identidade e Alçada de Verdade — Iteração 1)
...
Seu diretório de trabalho é: c:\Users\Felipe Barbosa\Documents\insight-compras\.agents\worker_u4_identidade_alcada_r1
...
Execute:
1. Eliminar seletor estático e CARTEIRAS_DEMO em CockpitPrincipal.tsx e conectar cockpit à carteira da sessão real (allowedSupplierIds). Garantir falha fechada para compradores sem carteira (grade vazia).
2. Garantir restrição server-side em /api/compras/route.ts.
3. Adicionar troca de senha e desativação de usuário na porta (src/lib/autenticacao/porta.ts) e nos dois provedores (demo.ts e supabase.ts), com ações na interface em /configuracoes/usuarios.
