/**
 * Repositório SUPABASE (PostgREST) do ciclo de aprendizado.
 * Camada: Aplicação (src/lib/aprendizado/provedores) — server-only.
 * Implementa a porta; a plataforma nunca importa este arquivo diretamente.
 */

import type { PerfilRotatividade } from "@core/dominio";
import type { StatusConfirmacao } from "@core/aprendizado";
import { sbInserir, sbSelecionar, sbUpsert } from "../supabase";
import {
  ConfirmacaoEntrada,
  FeedbackEntrada,
  ItemComparativo,
...
export class RepositorioAprendizadoSupabase implements RepositorioAprendizado {
  ...
  async listarComparativo(p: { tenantId: string; dias: number; filialId?: number; limite?: number }): Promise<ItemComparativo[]> {
    const filtroFilial = p.filialId ? `&filial_id=eq.${p.filialId}` : "";
    const limite = Math.min(5000, Math.max(1, p.limite ?? 2000));
    const linhas = await sbSelecionar<LinhaItemDb>(
      ...
      `select=id,snapshot_id,produto_id,sku,descricao,filial_id,custo,qtd_comprador,qtd_modelo,` +
        ...
        `&tenant_id=eq.${encodeURIComponent(p.tenantId)}` +
        `&aprendizado_snapshot.exportado_em=gte.${desdeIso(p.dias)}${filtroFilial}` +
  ...
  async listarItensParaConfirmar(p: { tenantId: string; dias: number; limite?: number }): Promise<ItemParaConfirmar[]> {
    const limite = Math.min(5000, p.limite ?? 3000);
    const linhas = await sbSelecionar<{
      ...
      `select=id,snapshot_id,produto_id,filial_id,qtd_comprador,aprendizado_snapshot!inner(exportado_em),aprendizado_confirmacao(status)` +
        `&tenant_id=eq.${encodeURIComponent(p.tenantId)}&elegivel=eq.true` +
        `&aprendizado_snapshot.exportado_em=gte.${desdeIso(p.dias)}&order=id.asc&limit=${limite}`
    );
    return linhas
  ...
  async listarLinhasCalibracao(p: { tenantId: string; dias: number }): Promise<LinhaCalibracaoDb[]> {
    const linhas = await sbSelecionar<{
      ...
      `select=perfil,consumo_diario,horizonte_dias,margem_aplicada,fator_calibracao,` +
        ...
        `&tenant_id=eq.${encodeURIComponent(p.tenantId)}&elegivel=eq.true` +
        `&aprendizado_snapshot.exportado_em=gte.${desdeIso(p.dias)}&limit=20000`
    );
    return linhas
  ...
  async publicarParametros(entrada: PublicacaoParametros): Promise<ParametrosPublicados> {
    const versao = gerarVersaoParametros();
    ...
    return { versao, margens: entrada.margens, fatorCalibracao: entrada.fatorCalibracao, publicadoPor: entrada.usuario, criadoEm: new Date().toISOString() };
  }
  ...
  async carregarParametrosPublicados(tenantId: string): Promise<ParametrosPublicados | null> {
    ...
    const l = linhas[0];
    return l
      ? { versao: l.versao, margens: l.margens, fatorCalibracao: Number(l.fator_calibracao), publicadoPor: l.publicado_por, criadoEm: l.created_at }
      : null;
  }
}
