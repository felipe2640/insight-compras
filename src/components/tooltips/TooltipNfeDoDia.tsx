...
export function TooltipNfeDoDia({
  entradas = [],
  delayDuration = 0,
  children,
}: PropsTooltipNfeDoDia) {
  // Se não há entradas hoje, renderiza apenas o elemento filho sem gatilhos nem tooltip
  if (!entradas || entradas.length === 0) {
    return <>{children}</>;
  }

  const totalPecasRecebidas = entradas.reduce(
    (acc, cur) => acc + cur.quantidadeEntrada,
    0
  );
